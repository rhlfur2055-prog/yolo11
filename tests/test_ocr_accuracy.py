...
# ── 상수 ──
# tests/ 로 이동되었으므로 PROJECT_ROOT 는 한 단계 위 (../).
BASE_DIR: Path = Path(__file__).resolve().parent
PROJECT_ROOT: Path = BASE_DIR.parent
IMG_DIR: Path = PROJECT_ROOT / "22"
DEFAULT_OUTPUT_DIR: Path = PROJECT_ROOT / "test_results"
...
def load_engine(verbose: bool):
    ...
    started_at = time.time()
    from yolo11_plate.plate_engine_pro import PlateEnginePro  # 지연 import (logging 설정 이후)
