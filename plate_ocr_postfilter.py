...
def is_valid_korean_plate(text: str) -> bool:
    ...
    # 구형 번호판 (지역명2자 + 숫자2자리 + 한글1자 + 숫자4자리)
    m_old = re.match(r"^(" + REGION_NAMES + r")(\d{1,2})([가-힣])(\d{4})$", cleaned)
    if m_old:
        plate_hangul = m_old.group(3)
        if plate_hangul not in VALID_HANGUL:
            return False
        return True

    # 신형 번호판: 한글 정확히 1자, 허용 목록 내
    if len(hangul) != 1:
        return False
...
def _immediate_reject(text: str) -> bool:
    ...
    hangul = [c for c in cleaned if "\uac00" <= c <= "\ud7a3"]
    if len(hangul) >= 2:
        # 구형 번호판 (지역명+숫자+한글+숫자) 패턴이면 통과
        if not re.match(r"^(" + REGION_NAMES + r")\d{1,2}[가-힣]\d{4}$", cleaned):
            return True
    # 한글 1자인데 숫자 구조 불일치
    if len(hangul) == 1:
